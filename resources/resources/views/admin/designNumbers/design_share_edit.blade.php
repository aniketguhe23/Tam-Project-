...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{url('admin/design-share-update')}}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="design_name">Select Design</label>
               <select  name="design"  class="form-control" required="">
                 <option value="{{$select_designs->id}}" selected="">
                  {{$select_designs->design_name}}
                 </option>
                 @foreach($desings as $desing)
                  <option value="{{$desing->id}}">{{$desing->design_name}}</option>
                  @endforeach
              </select>
            </div>
