...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.customerOrder.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.customer-orders.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('design') ? 'is-invalid' : '' }}" name="design_id" id="design_id" required>
                    @foreach($designs as $id => $design)
                        <option value="{{ $id }}" {{ old('design_id') == $id ? 'selected' : '' }}>{{ $design }}</option>
                    @endforeach
                </select>
